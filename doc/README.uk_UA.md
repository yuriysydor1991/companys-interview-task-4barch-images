...
Для того щоб переглянути команди побудови проекту у debug конфігурації необхідно переглянути скрипт `src/scripts/build-debug.sh` для побудови з більшістю функцій debug конфігурації (debug-символи, усі тести, санітайзери) або запустити за допомогою команди:
...
Для того щоб переглянути команди побудови проекту у конфігурації `Release` необхідно переглянути скрипт `src/scripts/build-release.sh` або безпосередньо запустити проект на побудову за допомогою нього:

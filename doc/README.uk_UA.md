...
Після успішного виконання даної команди у поточній відкритій директорї повинна з'явитись нова під директорія `companys-interview-task-4barch-images`. Отож відкрий її за допомогою команди `cd companys-interview-task-4barch-images`.

# Скрипти побудови

Дана секція містить ознайомлення з скриптами швидкої побудови. Більше інформації про доступні функції проекту можна знайти у подальших секціях документу.

## Скрипт швидка команда для побудови debug конфігурації

Для того щоб переглянути команди побудови проекту у debug конфігурації необхідно переглянути скрипт [src/scripts/build-debug.sh](src/scripts/build-debug.sh) для побудови з більшістю функцій debug конфігурації (debug-символи, усі тести, санітайзери) або запустити за допомогою команди:

```
# з кореневої директорії склонованого проекту

src/scripts/build-debug.sh
```

## Скрипт швидка команда release побудови 

Для того щоб переглянути команди побудови проекту у конфігурації `Release` необхідно переглянути скрипт [src/scripts/build-release.sh](src/scripts/build-release.sh) або безпосередньо запустити проект на побудову за допомогою нього:
...
Для того щоб запустити програму після її успішної побудови з параметром командного рядка для зміни поточної директорії роботи програми необхідно запустити:
...
Для встановлення `Qt6` бібліотеки необхідно виконати наступну команду:

```
# усі пакети для розробки за допомогою Qt6
...
Яка у свою чергу створює директорію `doc/barch-encoder-html` (вже додана до файлу `.gitignore`) що буде містити зібрану документацію у вигляді HTML-сторінок і ресурсів до них. Для того щоб відкрити і оглянути згенеровану документацію необхідно відкрити файл який лежить за шляхом `doc/barch-encoder-html/index.html` від кореня проекту. Директорія `barch-encoder-html` змінить назву у відповідності до зміни назви головного виконуваного файлу проекту-шаблону за допомогою встановлення нового значення для змінної `PROJECT_BINARY_NAME` у файлі `cmake/template-project-misc-variables-declare.cmake` або змінну `DOXYGEN_OUT_HTML_NAME` яка визначає ім'я для цілої цільової директорії для згенерованої HTML-документації.
...
Згенерований пакет повинен згенеруватись всередині кореня директорії побудови проекту. Наприклад, якщо ім'я проекту було не зміненим і його версія 0.8.0 тоді назва пакету може виглядати наступним чином: `barch-encoder-0.8.0-Linux.deb`.
...
Щоб увімкнути і запустити Docker контейнер разом з цільовим бінарним виконуваним файлом поточного проекту-шаблону необхідно встановити у значення `ON` змінну `ENABLE_DOCKER`, а також змінну `CONVERT_IMAGES_DIR` для передачі директорії з зображеннями за допомогою команди:
...
Ім'я `barch-encoder` являється ім'ям головного виконуваного бінарного файлу за замовчуванням. Його необхідно замінити у разі заміни ім'я за умовчанням у кореневому файлі `CMakeLists.txt` (змінні `PROJECT_NAME` або `PROJECT_BINARY_NAME`).
...
І ще раз, ім'я `barch-encoder` являється ім'ям головного виконуваного бінарного файлу за замовчуванням. Його необхідно замінити у разі заміни ім'я за умовчанням у кореневому файлі `CMakeLists.txt` (змінні `PROJECT_NAME` або `PROJECT_BINARY_NAME`).

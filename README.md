...
To be able to build and run the Docker container with current project' target binary file enable it's CMake target with setting the `ENABLE_DOCKER` and `CONVERT_IMAGES_DIR` (for the host images source directory) variables into value `ON` by using the next command:

```
# inside the project root directory

cmake -S . -B build -DENABLE_DOCKER=ON -DCONVERT_IMAGES_DIR=/path/to/images/dir/rw
```
